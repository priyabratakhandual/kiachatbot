...
{   
    ...
    public function verifyDealer(Request $request){
        $user = \DB::connection('demo')->table('vtiger_account')->join('vtiger_accountscf', 'vtiger_account.accountid', '=', 'vtiger_accountscf.accountid')->where('vtiger_accountscf.cf_854',strtoupper($request->dealer_code))->first();
        if($user){
    ...
    public function firstOrNewContact($firstname,$lastname,$email_id,$mobile,$organization){
        ...
        public function createTicket(Request $request){
              ...
              $dealer_info = \DB::connection('demo')->table('vtiger_account')->join('vtiger_accountscf', 'vtiger_account.accountid', '=', 'vtiger_accountscf.accountid')->where('vtiger_accountscf.cf_854',strtoupper($request->dealer_code))->select('vtiger_account.accountid')->first();
            // if(Session::has('vtiger_session')){
            //     $sessionId = Session::get('vtiger_session')[1];
              ...
              if($dealer_info){
                ...
                $data = array(
                    ...
                    'cf_878' => $request->product, //Solution Category
    ...
    public function createTicketOld(Request $request){
            ...
            //     $sessionId = Session::get('vtiger_session')[1];
              ...
              if($dealer_info){
                ...
                $data = array(
                    ...
                    'ticketpriorities' => $request->ticketpriorities,
                    'description' => $request->description,
                    'cf_982' => strtoupper($request->dealer_code),
                    'assigned_user_id' => '$vtiger->_userid',
                    'parent_id' => '11x'.$dealer_info->accountid,
                    'product_id' => '14x704', 
                    // 'cf_878' => 'Others',
    ...
    public function fetchTicketByTicketNumber(Request $request){
                 ...
                 $ticket_source = \DB::connection('demo')->table('vtiger_troubletickets')->where('ticket_no',$request->ticket_number)->first();
    ...
    public function fetchTicket(Request $request){
               $dealer = \DB::connection('demo')->table('vtiger_account')->join('vtiger_accountscf', 'vtiger_account.accountid', '=', 'vtiger_accountscf.accountid')->where('vtiger_accountscf.cf_854',strtoupper($request->dealer_code))->select('vtiger_account.accountid')->first();
